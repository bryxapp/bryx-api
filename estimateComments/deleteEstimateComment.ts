...
const deleteEstimateComment = async (context: Context, req: HttpRequest, decodedToken: KindeTokenDecoded): Promise<void> => {
  try {
    ...
    appInsights.defaultClient.trackException({
      exception: new Error("Delete estimate comment by id failed"), properties: { userId: decodedToken.sub, orgId: decodedToken.org_code, api: "Estimates" }
    });
    context.res = {
      status: 500,
      body: error.message
